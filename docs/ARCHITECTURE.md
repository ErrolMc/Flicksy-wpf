...
| --- | --- | --- |
| [Project](../Flicksy.VideoEditor/Project/Project.cs) | Document root. Static factories `CreateEmpty()` (4 tracks: 2 Video, 1 Overlay, 1 Audio) and `CreateStub()` (2 MediaClips on Video 1, 1 GraphicsClip on Overlay, 1 audio MediaClip on Audio — Video 2 stays empty, so it collapses out of the rendered timeline). | `ProjectSettings Settings`, `ObservableCollection<Track> Tracks` |
...
| [FileNameConverter](../Flicksy.VideoEditor/Converters/FileNameConverter.cs) | Maps a full file path to `Path.GetFileName`. Used by `ClipView`'s `MediaClip` template to label the clip with the source filename. |
