...
| --- | --- | --- |
| [Project](../Flicksy.VideoEditor/Project/Project.cs) | Document root. Static factories `CreateEmpty()` (4 tracks: 2 Video, 1 Overlay, 1 Audio), `CreateFromSourceFile(path)` (opens the file with `FFMediaToolkit` to probe duration, then `CreateEmpty()` + appends a single `MediaClip` at `TimelineStart=0` on the first video track), and `CreateStub()` (2 MediaClips on Video 1, 1 GraphicsClip on Overlay, 1 audio MediaClip on Audio — Video 2 stays empty, so it collapses out of the rendered timeline). | `ProjectSettings Settings`, `ObservableCollection<Track> Tracks` |
